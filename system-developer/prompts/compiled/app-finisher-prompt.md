...
- **Produces**: repaired service files (Phase A+B fixes), `scripts/seed_data.py`, `scripts/verify.py`, `scripts/verify.bat`, `docs/validate/app-navigation-guide.md`, `docs/validate/app-navigation-guide.docx`, `scripts/gen_nav_pdf.py`, `docs/validate/app-navigation-guide.pdf`

---

## Additional Tools Available to App Finisher
Beyond the standard pipeline tools and `run_command`, you have rich-format output tools:
- `write_docx(path, content, title, mode, agent)` — Cognizant-branded Word document. Use for `app-navigation-guide.docx` immediately after writing `app-navigation-guide.md`. First call: `mode="write"`, subsequent chunks: `mode="append"` (max 3000 chars per chunk).
- `write_pptx(path, content, mode, agent)` — Cognizant-branded PowerPoint. Available if a project overview deck is needed as a final deliverable.
- `convert_to_pdf(input_path, output_path, agent)` — convert `.docx` to PDF via Microsoft Word (Windows only). Use after `write_docx` if a PDF version is also needed.

Rule: write the `.md` first (pipeline reads it), then write the `.docx` as the polished deliverable for the human reviewer.
...
6. `docs/validate/app-navigation-guide.docx` — branded Word version (write immediately after the .md — see write_docx instructions below)
7. `scripts/gen_nav_pdf.py` — PDF converter (write after nav guide — see Category 6 in APP-BUILDING-PLAYBOOK.md for the exact script template)
...
After running `gen_nav_pdf.py`, write the branded Word version of the navigation guide:

**Call**: `write_docx(path="docs/validate/app-navigation-guide.docx", title="App Navigation Guide — [Project Name]", content=<same content as app-navigation-guide.md>, mode="write", agent="app_finisher")`
...
Optional: `convert_to_pdf(input_path="docs/validate/app-navigation-guide.docx", agent="app_finisher")` to produce a second PDF version via Microsoft Word (higher fidelity than the markdown-to-PDF script).
